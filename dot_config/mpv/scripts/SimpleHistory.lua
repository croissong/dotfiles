...
local available_filters =
	{ "all", "recents", "distinct", "playing", "protocols", "fileonly", "titleonly", "timeonly", "keywords" }
...
function display_list(filter, sort, action)
	...
	end --1.1.6# fix osc-idlescreen (value was yes for some reason)
	list_drawn = true
	if not search_active then
		get_list_keybinds()
	end
...
function history_blacklist_check()
	if not o.history_blacklist[1] or #o.history_blacklist == 1 and o.history_blacklist[1] == "" then
		return false
	end
	...
	elseif not starts_protocol(protocols, filePath) then
		if
			has_value(o.history_blacklist, filePath:match("^(.-)([^\\/]-)%.([^\\/%.]-)%.?$"), nil)
			or has_value(o.history_blacklist, filePath:match("^(.-)([^\\/]-)%.([^\\/%.]-)%.?$"):gsub("\\$", ""), nil)
		then
			msg.info(blacklist_msg)
			return invertable_return[1]
		elseif
			has_value(o.history_blacklist, filePath:match("%.([^%.]+)$"), nil)
			or has_value(o.history_blacklist, "." .. filePath:match("%.([^%.]+)$"), nil)
		then
			msg.info(blacklist_msg)
			return invertable_return[1]
			...
			for i = 1, #o.history_blacklist do --1.1.2# loop through blacklisted items, if the blacklist ends with * and it is a match after subbing of the current filePath then log it. #and additionally if it is the exact same path then ignore it.
				if
					string.lower(filePath):match(string.lower(o.history_blacklist[i]))
					and o.history_blacklist[i]:sub(-1, #o.history_blacklist[i]) == "*"
					and string.lower(o.history_blacklist[i]:sub(1, -2)) ~= string.lower(filePath):match("(.*[\\/])")
				then
					msg.info(blacklist_msg)
					return invertable_return[1]
				end
	...
	elseif starts_protocol(protocols, filePath) then
		...
		elseif filePath:find("https?://") == 1 then
			...
			local different_checks = {
				different_check_temp,
				filePath:match("https?://w?w?w?%.?([%w%.%:]*)"),
				filePath:match("https?://([%w%.%:]*)"),
				filePath:match("(https?://[%w%.%:]*)"),
			}
			for i = 1, #different_checks do
				if
					different_checks[i] and has_value(o.history_blacklist, different_checks[i], nil)
					or different_checks[i] .. "/" and has_value(o.history_blacklist, different_checks[i] .. "/", nil)
				then
					msg.info(blacklist_msg)
					return invertable_return[1]
				end
...
function mark_chapter()
	if not o.mark_history_as_chapter then
		return
	end
	...
	if o.resume_option == "notification" or o.resume_option == "force" then
		...
		if logged_time > 0 then
			...
			if o.resume_option == "notification" then
				if
					percentage > o.resume_option_threshold and percentage < (100 - o.resume_option_threshold)
					or o.resume_option_threshold == 0
				then
					mp.osd_message(
						"⌨ ["
							.. string.upper(o.history_resume_keybind[1])
							.. "] Resumes To"
							.. o.time_seperator
							.. format_time(
								logged_time,
...
mp.add_hook(
	"on_unload",
	9,
	function() --1.1.3# get the LogTime only when using on_unload because big functions do not run fully in here
		logTime = (mp.get_property_number("time-pos") or 0)
	end
)
mp.register_event(
	"end-file",
	function() --1.1.3# use end-file instead so that it doesn't cause crash while seeking ( i am able to run big functions here)
		if not incognito_mode then
			if autosaved_entry == true then
				delete_log_entry_specific("last", filePath, 0)
			end
...
mp.observe_property("idle-active", "bool", function(_, v)
	...
	if
		v and o.auto_run_incognito_mode and not incognito_auto_run_triggered
		or not v and o.auto_run_incognito_mode and not incognito_auto_run_triggered
	then
