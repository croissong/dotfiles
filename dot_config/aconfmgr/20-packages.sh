...
AddPackage ttf-arphic-uming # CJK Unicode font Ming style
...
AddPackage ttf-hannom # Chinese and Vietnamese TrueType fonts
AddPackage ttf-indic-otf # Indic Opentype Fonts collection
AddPackage units # converts between different units
...
AddPackage --foreign bato-bin # Small program to send battery notifications
...
AddPackage --foreign libfixposix # Thin wrapper over POSIX syscalls
AddPackage --foreign libgccjit # Just-In-Time Compilation using GCC.
...
AddPackage --foreign mattermost-desktop-bin # Mattermost Desktop for Linux (binary)
